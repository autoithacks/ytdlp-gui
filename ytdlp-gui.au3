...
func dlIn1080p()
	...
	$params = " -f bv*[height<=1080][ext=mp4]+ba --merge-output-format mp4 --sponsorblock-remove all -o "&$dlpath&"\\%(title)s-%(id)s.%(ext)s" 	
...
Func startDL($link, $params, $log)
	...
	GUICtrlSetData($log, $line)
			...
			if StringInStr($gesamtlog,"Requested format is not available") and $dlbitrate <> "360p" then
